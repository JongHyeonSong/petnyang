...
let FONT_PATH = "";
const FONT_DIR = path.join(__dirname, "fonts/HMKMRHD.TTF");
...
function maker(opt) {
  ...
  for (const [idx, commnet] of comments.entries()) {
    const mnts = `drawtext=text='${commnet}':x=${startPos.x}:y=${
      startPos.y + idx * LINE_GAP
    }:${DEFAULT_OPTION}:enable='between(t,${timeSlice.start},${
      timeSlice.end
    })'`;
    returnArr.push(mnts);
  }
